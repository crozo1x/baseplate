...
function sanitizeBuilderState(builder) {
  if (!isPlainObject(builder)) {
    return { ideaText: '', chips: [], plan: null, scriptsTested: {} };
  }
  const ideaText = typeof builder.ideaText === 'string' ? builder.ideaText.slice(0, 4000) : '';
  const chips = Array.isArray(builder.chips)
    ? builder.chips.filter((c) => typeof c === 'string').slice(0, 50)
    : [];
  const plan = isPlainObject(builder.plan) ? builder.plan : null;
  const scriptsTested = isPlainObject(builder.scriptsTested) ? builder.scriptsTested : {};
  return { ideaText, chips, plan, scriptsTested };
}

function sanitizeConfig(config) {
  if (!isPlainObject(config)) {
    return { projectFolder: null, widgets: [], builder: sanitizeBuilderState(null) };
  }
  const projectFolder =
  ...
  const widgets = Array.isArray(config.widgets)
    ...
    : [];
  const builder = sanitizeBuilderState(config.builder);
  return { projectFolder, widgets, builder };
}
