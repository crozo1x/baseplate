...
function normalizeShell(shell, defaultShell) {
  const hasRequestedShell = typeof shell === 'string' && shell.trim().length > 0;
  const shellPath = hasRequestedShell ? shell.trim() : defaultShell;
  ...
  if (hasRequestedShell && path.basename(shellPath) !== shellPath) {
    return { ok: false, error: 'Shell must be a named launch profile' };
  }
...
function sanitizeTerminalIdPayload(payload) {
  if (!isPlainObject(payload) || typeof payload.id !== 'string' || !SESSION_ID_RE.test(payload.id)) {
    return { ok: false, error: 'Invalid terminal id' };
  }
  return { ok: true, id: payload.id };
}

function sanitizeTerminalInputPayload(payload) {
  const idResult = sanitizeTerminalIdPayload(payload);
  if (!idResult.ok) return idResult;
  if (typeof payload.data !== 'string') {
    return { ok: false, error: 'Invalid terminal input' };
  }
  return { ok: true, id: idResult.id, data: payload.data };
}

function sanitizeTerminalResizePayload(payload) {
  const idResult = sanitizeTerminalIdPayload(payload);
  if (!idResult.ok) return idResult;
  return {
    ok: true,
    id: idResult.id,
    cols: toBoundedInt(payload.cols, 80, 20, 400),
    rows: toBoundedInt(payload.rows, 30, 5, 200),
  };
}
