...
function renderActiveSessions(container) {
  function draw(sessions) {
    if (!sessions || sessions.length === 0) {
      container.innerHTML = '<p class="widget-empty">Sessions you open will show up here.</p>';
      return;
    }
    container.innerHTML =
...
function renderGitStatus(container) {
  async function draw() {
    ...
    if (!folder) {
      container.innerHTML = '<p class="widget-empty">Set a project folder to see Git status.</p>';
      return;
    }
